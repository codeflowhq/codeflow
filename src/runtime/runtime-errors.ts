...
const FRIENDLY_PATTERNS: FriendlyPattern[] = [
  ...
  { pattern: /Missing browser runtime config at /i, message: "Browser runtime config is missing or could not be loaded." },
  { pattern: /Failed to load Pyodide script/i, message: "Pyodide could not be loaded. Check your network connection and try again." },
  { pattern: /Pyodide did not expose loadPyodide on window/i, message: "Pyodide did not initialize correctly in the browser." },
  {
    pattern: /InvalidWheelFilename:.*['"]?([^'"\s]+\.whl|[^'"\s]+)['"]?/i,
    message: "A browser runtime wheel filename is invalid. Use the original wheel filename generated by Python packaging.",
  },
  {
    pattern: /(https:\/\/github\.com\/[^\s]+\/releases\/latest\/download\/[^\s]+\.whl).*(404|Not Found)|\b404\b.*(https:\/\/github\.com\/[^\s]+\/releases\/latest\/download\/[^\s]+\.whl)/i,
    message: "A required browser runtime wheel could not be downloaded from GitHub Releases. Publish the wheel release asset or update the runtime wheel URL.",
  },
